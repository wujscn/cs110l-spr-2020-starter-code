...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_link_path = fs::read_link(fd_path).ok()?;
        let fd_link_name = OpenFile::path_to_name(fd_link_path.to_str()?);
        // solve fd info
        let fd_info_path = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fd_info = fs::read_to_string(fd_info_path).ok()?;
        let fd_cursor = OpenFile::parse_cursor(&fd_info)?;
        let fd_access_mode = OpenFile::parse_access_mode(&fd_info)?;

        let of = OpenFile::new(fd_link_name, fd_cursor, fd_access_mode);
        Some(of)
    }
